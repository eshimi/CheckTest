...
def build_overall_comment(examinee, scene_scores, comp_comments=None):
    ...
    if pct >= 80:
        intro = (
            f"総合得点率{pct}%、素晴らしい結果です。"
            f"情報の収集・分析・判断・伝達の各場面において、"
            f"災害対策員として高い水準の対応力を発揮していただきました。"
            f"不確実な状況下でも状況を整理し、班として取るべき行動を的確に選択する力が"
            f"シーンを通じて一貫して示されており、その実践力は大きな強みです。"
        )
    elif pct >= 65:
        intro = (
            f"総合得点率{pct}%を獲得されました。"
            f"災害対策員として求められる情報共有・応急復旧対応の基本的な枠組みは"
            f"しっかりと身についており、安定した対応力を発揮されています。"
            f"今後の実践をさらに重ねることで、着実な成長が期待できます。"
        )
    elif pct >= 50:
        intro = (
            f"総合得点率{pct}%という結果でした。"
            f"情報共有・応急復旧の基本的な方向性はしっかり捉えられており、"
            f"行動のベースはできています。"
            f"いくつかの場面で判断の精度や伝達の明確さをさらに磨いていくと、"
            f"より力強い対応につながっていきます。"
        )
    else:
        intro = (
            f"総合得点率{pct}%という結果でした。"
            f"今回の試験では、いくつかの場面で情報の収集・分析・共有のつなぎ目に"
            f"伸びしろが見えています。"
            f"裏を返せば、そこを少し意識するだけで大きく変われるポイントでもあります。"
            f"このフィードバックをヒントに、ご自身のペースで取り組んでみてください。"
        )
    ...
    if strong_comps:
        ...
        intro += (
            f"特に{strong_names}は高い水準を示しており、"
            f"この強みは今後の災対活動においても大きな武器になります。"
        )

    # ② 成長へのエール：実際に低いコンピテンシーのみ言及
    if grow_comps:
        ...
        growth_comment = (
            f"今後は、不確実性が高い局面や複数の課題が重なる場面でも"
            f"現在の判断精度を安定して発揮することが、さらなる成長につながります。"
            f"ご自身の災対経験で気づいた視点を、ぜひ周囲にも共有していただければ嬉しいです。"
        )
    ...
    desc = {
        "コミュニケーション": {
            "high": (
                ...
                "情報の受け手が意思決定に使えるよう要点を絞ったエスカレーションや双方向のやり取りを引き続き大切にしていただくことで、さらに磨きがかかります。"
            ),
            "mid": (
                "班内の情報共有・報告は概ね適切で、クリティカルな情報を伝達する基盤はしっかりできています。"
                "相手が理解・活用できているかを確認しながら進める双方向のコミュニケーションを少し意識してみると、連携がさらにスムーズになるでしょう。"
            ),
            "low_mid": (
                "情報を伝えようとする行動はしっかり取れています。"
                "「何を・誰に・いつ伝えるか」を少し整理してみると、伝達がより的確になり、チームとしての動きもさらによくなります。"
            ),
            "low": (
                "まずは自班内で「今何が起きているか・何が必要か」を一言でまとめて伝えることから始めてみましょう。"
                "そのひと言の積み重ねが、連携の土台になります。"
            ),
        },
        "情報収集": {
            "high": (
                "参集後の状況把握において、必要な情報を能動的・多角的に収集し、複数の情報源を組み合わせて状況を先手先手で把握できています。"
                "収集した情報を班内で共有し応急復旧の判断に直結させる流れを、今後もぜひ大切にしてください。"
            ),
            "mid": (
                "公式情報源の確認や関係者への聴取など基本的な収集行動はしっかり取れており、状況把握の基盤ができています。"
                "他班・外部機関・現場など多角的なルートも少しずつ広げていくと、より先手先手の対応につながっていくでしょう。"
            ),
            "low_mid": (
                "与えられた情報を受け取る力はあります。"
                "そこに「今何を確認すべきか」を自分から問いかけるひと手間を加えてみると、対応の幅が広がります。参集時の確認先（上位組織・他班・現場）をあらかじめイメージしておくと動きやすくなります。"
            ),
            "low": (
                "まずは参集時に確認すべき情報（被害状況・班の状況・対応済み事項）を3つほど頭に入れておくことから始めてみましょう。"
                "小さなルーティンが、いざというときの素早い行動につながります。"
            ),
        },
        "情報分析": {
            "high": (
                "収集情報の中からクリティカルな事象を特定し、事実と推測を区別しながら影響範囲・優先度を的確に判断できています。"
                "分析の根拠を班内で共有し、組織全体の優先順位判断に活かせる形で発信するという強みを、今後もぜひ活かしてください。"
            ),
            "mid": (
                "重大な影響を及ぼす情報を見極める基本的な分析はできており、判断力の土台はしっかりしています。"
                "「なぜそれがクリティカルなのか」という根拠を一言添える習慣が身につくと、分析の説得力がぐっと増します。"
            ),
            "low_mid": (
                "情報を整理しようとする意識はしっかりあります。"
                "「影響範囲の広さ」と「緊急度」の2軸で情報を見てみると、優先すべきものが自然と見えてきます。少し試してみてください。"
            ),
            "low": (
                "「この情報は確かなのか」「どこまで影響が広がるか」「どれくらい急ぐか」という3つの問いを習慣的に持つことで、判断がしやすくなります。"
                "焦らず、一つひとつ確かめていく姿勢が大切です。"
            ),
        },
        "想像・先読み": {
            "high": (
                "収集情報からリスク・影響を想像して先読みし、班として取るべき対応を組織全体の視点で判断できています。"
                "想定外の事態が発生した際の代替対応も視野に入れておくと、状況変化への対応力がさらに高まります。"
            ),
            "mid": (
                "次に起こりうる事態を意識した対応は取れており、基本的なシナリオ想定は身についています。"
                "想定した事態のリスク・影響を組織全体の視点で評価し、それを意思決定につなげる部分もぜひ意識してみてください。"
            ),
            "low_mid": (
                "目の前の対応はしっかりできています。"
                "情報を受け取ったときに「1手先・2手先でどうなるか」をちょっと想像してみる習慣をつけると、先手の行動がとりやすくなります。"
            ),
            "low": (
                "目の前の対応に集中するのは大切なことです。"
                "余裕ができたタイミングで「この後どうなりそうか」を少し想像してみることを積み重ねると、じわじわと先読みの力がついてきます。"
            ),
        },
        "計画": {
            "high": (
                "優先順位の設定・役割分担・タイムライン設計が的確で、制約がある中でも実行可能な応急復旧計画を組み立てられています。"
                "計画立案の段階で状況変化を想定した代替案もあわせて準備しておくと、実行段階での判断がさらにスムーズになります。"
            ),
            "mid": (
                "基本的な行動計画の立案と優先順位の設定は概ね適切で、班内の応急復旧を進める計画力の基盤があります。"
                "「いつまでに・誰が・何を完了させるか」を少し具体的にしていくと、計画がより動きやすいものになるでしょう。"
            ),
            "low_mid": (
                "対応の方向性はしっかり考えられています。"
                "その方向性を「誰が・いつまでに」という形で少し具体化してみると、チームとして動きやすくなります。完璧でなくてもOK、まず形にしてみることが大切です。"
            ),
            "low": (
                "まず「優先度が高そうな対応を3つ挙げて、それぞれ誰がやるかを決める」ことから始めてみましょう。"
                "その小さな一歩が、計画を立てる感覚を育ててくれます。"
            ),
        },
    }
    # 半角カタカナ→全角正規化マップ
...
def build_scene_comments(scene_scores):
    ...
    for scene, ss in scene_scores.items():
        ...
        if pct >= 80:
            opening = f"得点率{pct}%、このシーンでは非常に高い水準を発揮されました。"
            tone = "状況判断と対応の質が高く、シナリオ全体を通じて的確な行動選択が光りました。"
        elif pct >= 65:
            opening = f"得点率{pct}%を獲得されました。"
            tone = "基本的な対応方針をしっかり捉えており、安定した対応力を発揮されています。"
        elif pct >= 50:
            opening = f"得点率{pct}%という結果でした。"
            tone = "対応の方向性は掴めており、いくつかの場面でさらに工夫を加えるとより良い結果につながります。"
        else:
            opening = f"得点率{pct}%という結果でした。"
            tone = "このシーンには難しい状況判断が多く含まれていました。問題別のコメントに改善のヒントがありますので、ぜひ参考にしてみてください。"

        strength = ""
        if best_comp and best_comp[1]["pct"] >= 70:
            strength = f"特に「{best_comp[0]}」（{best_comp[1]['pct']}%）での対応が光っていました。"

        grow = ""
        if low_comp and low_comp[1]["pct"] < 70 and (not best_comp or low_comp[0] != best_comp[0]):
            grow = f"「{low_comp[0]}」はさらに伸びしろがある部分です。少しずつ意識してみましょう。"
