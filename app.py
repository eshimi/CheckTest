...
def build_overall_comment(examinee, scene_scores):
    ...
    if pct >= 80:
        intro = (
            ...
            f"「上級者のありたい姿」を、複数のシーンにわたって発揮できています。"
        ...
        intro = (
            f"総合得点率{pct}%。"
            f"クリティカルな情報を見極め、分かりやすく伝える「中級者のありたい姿」の水準に達しています。"
            f"班での情報共有・応急復旧において状況を把握し適切に伝達する基盤は身についていますが、"
            f"先読みや組織全体を俯瞰した判断という上級者レベルには、まだ伸びしろがあります。"
        )
    elif pct >= 50:
        intro = (
            f"総合得点率{pct}%。"
            f"情報共有・応急復旧の基本的な枠組みは理解できていますが、"
            f"クリティカルな情報を判断して分かりやすく伝える「中級者のありたい姿」には"
            f"まだ達していない部分があります。"
    ...
    if grow_comps:
        ...
        if pct >= 65:
            gap_desc = "上級者のありたい姿である「先読みと組織全体を俯瞰した意思決定」"
        else:
            gap_desc = "中級者のありたい姿である「クリティカルな情報の判断と分かりやすい伝達」"
        growth_comment = (
    ...
    desc = {
        "コミュニケーション": {
            "high": (
                ...
                "上級者として、情報の受け手が意思決定に使えるよう要点を絞ったエスカレーションを徹底し、不明点は確認して双方向のやり取りを意識的に維持することが引き続き重要です。"
            ),
            "mid": (
                "班内の情報共有・報告は概ね適切で、クリティカルな情報を伝達する中級者レベルの基盤は確認できます。"
        ...
        "情報収集": {
            "high": (
                ...
                "上級者として、収集した情報を班内で共有し応急復旧の判断に直結させるまでの一連の流れを、引き続き精度高く維持することが求められます。"
            ),
            "mid": (
                "公式情報源の確認や関係者への聴取など基本的な収集行動は取れており、状況把握の基盤はあります。"
                "他班・外部機関・現場など多角的なルートから能動的に情報を集める行動が不十分な場面があり、この点の強化が上級者レベルへのステップです。"
            ),
            "low_mid": (
        ...
        "情報分析": {
            "high": (
                ...
                "上級者として、分析の根拠を班内で共有し、組織全体の優先順位判断に活かせる形で発信することを引き続き意識してください。"
            ),
            "mid": (
                "重大な影響を及ぼす情報を見極める基本的な分析はできており、中級者レベルの判断力は確認できます。"
        ...
        "想像・先読み": {
            "high": (
                ...
                "上級者として、想定外の事態が発生した際の代替対応も事前に考慮しておくことで、状況変化への対応力をさらに高められます。"
            ),
            "mid": (
                "次に起こりうる事態を意識した対応は取れており、基本的なシナリオ想定は身についています。"
                "想定した事態のリスク・影響を組織全体の視点で評価し、優先順位をつけて意思決定につなげる部分が、上級者との差となっています。"
            ),
            "low_mid": (
        ...
        "計画": {
            "high": (
                ...
                "上級者として、計画立案の段階で状況変化を想定した代替案も準備しておくことで、実行段階での判断をさらに迅速化できます。"
            ),
            "mid": (
                "基本的な行動計画の立案と優先順位の設定は概ね適切で、班内の応急復旧を進める計画力の基盤はあります。"
                "「いつまでに・誰が・何を完了させるか」という時間軸と役割分担の明確化が不十分な場面があり、この部分の具体化が上級者レベルへの課題です。"
            ),
            "low_mid": (
