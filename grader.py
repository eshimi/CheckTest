...
【採点解釈ルール（採点基準の読み方）】
採点基準に記載された要件を判断する際は、以下の解釈ルールに従うこと。

■ 施設名・組織名の明示は必須としない
採点基準に「本社（大手町プレイス）」「特定の拠点名」などが例示されていても、
回答にその固有名詞がなくても、概念として同等の内容（例：「電源確保できる場所への移動」）が
書かれていれば要件を満たすとみなす。
固有名詞の不記載を理由に減点してはならない。

■ 「今後の見通し」の情報収集には時間軸のある情報も含む
採点基準で「今後の見通し」「状況が悪化するのか改善するのか」の情報収集が求められている場合、
以下もすべてこれに該当する：
・燃料枯渇想定時間・補給手配の見込み時間
・サービス断が発生するまでの残り時間の把握
・復旧や回復に要する見込み時間の収集
これらの時間軸情報が収集されていれば「今後の見通しの情報収集」として3点要件を満たす。

■ 採点は「核心的要素の有無」で判断する
採点基準の各点数に相当する核心的な行動・思考が回答に含まれていれば、
表現の違いや記述の網羅性の不足を理由に点数を下げてはならない。
「理想的な回答に比べて不足している要素がある」だけでは減点の根拠にならない。"""
