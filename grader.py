...
【クロス評価の目的】
1問ずつの採点では捉えられなかった「他の回答に書かれた内容が、評価基準を満たす証拠となる」ケースを救済する。
例：問Bでは言及されなかった「班長への連絡」が、同シーンの問Aの回答に明確に書かれている場合、問Bの該当コンピテンシーを格上げする。

【格上げのルール】
- 格上げは最大1点（例：1点→2点、2点→3点）
- 証拠となる回答が明確に当該コンピテンシーの要件を満たしている場合のみ格上げ
- 強制0点（forced_zero=true）の問題は格上げしない
- 見直しが不要な場合は upgrades を空リストにする"""
...
同一シーン内で「別の問題の回答が当該問題の評価基準を満たす証拠となる」ケースがあれば、スコアを格上げしてください。
