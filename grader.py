...
・「〜か確認する」「〜を確認する」「〜かどうか確認する」等の確認行為の記述のみで、
  推測・想定表現（「〜の可能性がある」「〜が想定される」「〜だろう」等）が一切ない場合
  ※「原因を確認する」は情報収集であり「原因を想定する」ではない。確認行為は1点要件（背景を想定しようとしている）も満たさない
...
# 採点例・注記パターン（「→X点」「　X点」行末、または「〜の場合はX点」等）
_SCORING_NOTE_PATTERNS = [
    re.compile(r'[→　]([0-3]点)\s*$'),
    re.compile(r'(?:の場合は|ではあるが|ではないが|とみなし|として)\s*[0-3]点'),
    re.compile(r'[0-3]点\s*$'),  # 行末に点数のみで終わる行
]


def _extract_scoring_notes(rubric_text: str) -> list:
    """
    Rubricテキストから「→X点」「〜　X点」「〜の場合はX点」等の採点例・注記を抽出する。
    ルール8の適用対象となる行を返す。
    """
    notes = []
    for line in rubric_text.split('\n'):
        ...
        if not stripped:
            continue
        # 「X点：」で始まる行（基準本文）は除外、注記・例示のみ対象
        if re.match(r'^[0-3]点[：:]', stripped):
            continue
        for pat in _SCORING_NOTE_PATTERNS:
