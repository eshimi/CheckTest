...
■ 情報収集・計画は「核心性」で判断する
- 核心的な確認項目・対応フローの論理的筋道が示されていれば3点を与える
- 「この項目も挙げられればより良かった」程度の網羅性不足は減点しない
- 主要な確認事項と対応手順の一貫性が取れていれば、細部の列挙が不足していても満点とする

■ 想像・先読み・情報分析は「質」で判断する
- 問われている本質的視点（リスクの先読み・クリティカル情報の識別・深い分析）が明確に示されている場合に得点する
- 表面的・付随的な言及のみで核心的視点が欠けている場合は0点とする
- 「少し触れている」だけでは1点も与えない

■ 部分点（1点）の扱い
- 1点は「採点基準の中核要件を部分的には満たしているが、重要な要素が欠けている」場合に限定する
- 採点基準の本質的要件を満たしていなければ0点とする
...
【クロス評価の目的と基本姿勢】
受験者はシーン全体を通じて災害対応を考えている。1問ずつの独立採点では「その問題に書いていないが、別の問題で書いている」ことが評価されない。
シーン全体を通じて受験者が当該コンピテンシーの行動・思考を示していれば、個別問題の記述が不足していても積極的に評価する。

【格上げの判断基準（積極的に格上げすること）】
以下のいずれかに該当すれば格上げする：
① 同シーンの別問題の回答に、該当問題の採点基準で求められている内容・行動が書かれている
② 受験者がシーン全体を通じて、当該コンピテンシーの意識・行動を示していると読み取れる
③ 該当問題の回答では直接言及がないが、別問題の回答から「やっている・意識している」と合理的に推測できる
...
print("=== GRADER VERSION 2026-05-29-FUJITA LOADED ===", flush=True)
